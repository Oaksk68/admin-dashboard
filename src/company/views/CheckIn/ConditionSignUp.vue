    ...
    <div class="relative h-auto bg-white rounded-tr-3xl rounded-b-3xl md:p-10 pb-5">
      ...
      <div class="grid gap-2 md:flex md:gap-4 w-full pb-8 mt-4">
        <div class="relative">
          <select
            ...
            placeholder="" v-model="filter.listNum">
      ...
      <BaseModal :isOpen="popUp" @close-modal="closeModal" @submit="submit" :classList="'w-full md:w-[40vw]'">
        ...
        <template #container>
          <div class="">
            <form action=""
              class="flex flex-col w-full items-start md:justify-center md:items-center gap-5 text-primary pr-4 md:pr-0 mb-8 md:mb-0">
        ...
        <table class="w-full border-collapse text-sm text-left text-primary">
          ...
          <tbody class="text-center">
            <tr v-for="(data, i) in tableData" :key="data.id" class="bg-white">
              ...
              <td class="px-6 py-4 capitalize">{{ data.finetype.name }}</td>
              ...
              <td class="px-6 py-4 text-right flex">
                <div @click="editCondition(data.id)"
                ...
                <div @click="$deleteData('condition', data.id)"
      ...
      <div id="accordion" class="block md:hidden border-b mb-4">
        <div v-for="(data, index) in tableData" :key="index" class="border-t py-2 pl-5 pr-3" id="accordion">
          <button @click="this.$toggleAccordion(index, 'conditionSignUp')" class="flex items-center w-full gap-3">
            <font-awesome-icon :id="`conditionSignUp-chevron-${index}`" class="bg-origin p-2 text-white rounded-full"
      ...
      <div class="flex justify-end items-center gap-2 text-sm text-h1mute mt-5 md:pr-0 pr-4">
...
export default {
  ...
  methods: {
    ...
    async getPaginateNo() {
      await this.axios
        .post("/conditions?page=" + this.page, this.filter)
        .then((response) => {
    ...
    async submit() {
      if (this.form.id) {
        await this.axios
          .post("condition/update/" + this.form.id, this.form)
          .then((response) => {
        ...
        await this.axios
          .post("condition/create", this.form)
          .then((response) => {
            if (response.data.success) {
              this.$swal({
                icon: "success",
