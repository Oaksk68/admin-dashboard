    ...
    " class="text-[2em]" />
      ...
      <div id="accordion" class="block md:hidden border-b mb-4">
        <div v-for="(item, index) in tableData" :key="index" class="border-t py-2 pl-5 pr-3 text-sm" id="accordion">
          <button @click="this.$toggleAccordion(index, 'monthlyAttendance')" class="flex items-center w-full gap-3">
            <font-awesome-icon :id="`monthlyAttendance-chevron-${index}`" class="bg-origin p-2 text-white rounded-full"
