<template>
  <main class="w-full bg-calendarBg">
    ...
    <div class="overflow-auto">
      <div class="h-auto bg-white rounded-tr-3xl rounded-b-3xl p-6 md:px-12 md:py-[46px]">
        <div class="flex justify-between items-center mb-[45px]">
          <div class="flex gap-4">
            <div class="inline-block h-8 w-1 bg-origin rounded-3xl"></div>
            <h1 class="text-primary text-3xl">{{ $t('organization_info') }}</h1>
          </div>
          <div @click="editInformation = true" v-if="!editInformation"
            class="p-2 rounded-full border-2 bg-chooser cursor-pointer ease-in-out duration-200 hover:bg-slate-200">
        ...
        <form class="flex flex-col gap-2" action="">
          <div v-for="(leave, index) in formSetting.onleavedays" :key="index"
            class="grid md:flex text-primary text-18 mb-2">
            <label v-if="this.$store.getters.getAppLanguage == 'th'" :for="leave.day" class="inline-block md:w-1/4">
              {{ leave.onleavetype.name_th }}
            </label>
            <label v-if="this.$store.getters.getAppLanguage == 'en'" :for="leave.day"
              class="inline-block md:w-1/4 capitalize">
              {{ leave.onleavetype.name_en }}
            </label>
            <span v-if="!editInformation" class="w-2/4 pb-4">{{
              leave.day
            }}</span>
            <input :id="leave.day" v-else
              class="w-full md:w-2/4 border rounded-xl py-2 px-4 text-gray-700 transition ease-in-out duration-500 focus:outline-none focus:bg-white focus:border-sky-500"
              v-model="testForm[leave.onleavetype.type]" />
          </div>
          <div class="grid md:flex text-primary text-18 mb-5">
            <label for="line_group" class="inline-block md:w-1/4">{{ $t('lineCode') }}</label>
            <span v-if="!editInformation" id="line_group" class="w-2/4 pb-4">{{
              formSetting.line_group
            }}</span>
            <input name="line_group" id="line_group" v-model="formSetting.line_group" v-else type="text"
              class="w-full md:w-2/4 rounded-xl" />
          </div>
          <div class="grid md:flex text-primary text-18 mb-5">
            <label for="createdDate" class="inline-block md:w-1/4">วันที่ลงทะเบียน</label>
            <span v-if="!editInformation" class="w-2/4 pb-4" id="createdDate">{{
              this.$moment(formSetting.created_at).format("DD-MM-YYYY")
            }}</span>
            <VueDatePicker name="createdDate" id="createdDate" :format="$format" v-else v-model="formSetting.created_at"
              class="w-full md:w-2/4" input-class-name="text-muted rounded-xl focus:ring-blue-500 focus:border-blue-500"
              :enable-time-picker="false" />
          </div>
          <div class="grid md:flex text-primary text-18 mb-5">
            <label for="status" class="inline-block md:w-1/4">สถานะ</label>
            <span name="status" id="status" class="flex justify-start items-center gap-1 text-lime-300">
              <img class="w-4 h-4" src="@/assets/images/active.svg" alt="" />
              <p>ใช้งาน</p>
            </span>
          </div>
          <div class="grid md:flex text-primary text-18 mb-5">
            <label for="toggle" class="inline-block md:w-1/4">อนุญาตให้พนักงานลงทะเบียน</label>
            <span name="toggle" id="toggle" class="w-2/4 pb-4">
              <Toggle class="toggle-blue" v-model="regStatus" trueValue="1" falseValue="0" :disabled="!editInformation" />
            </span>
          </div>
          <div class="grid md:flex text-primary text-18 mb-5">
            <label for="secret_code" class="inline-block w-1/4">{{ $t('secret_code') }}</label>
            <span v-if="!editInformation" id="secret_code" class="w-2/4 pb-4">{{
              formSetting.allow_register_code
            }}</span>
            <input v-else type="text" id="secret_code" v-model="formSetting.allow_register_code"
              class="w-full md:w-2/4 rounded-xl px-4" :placeholder="$t('secret_code')" />
          </div>
        </form>
        <div v-if="editInformation"
          class="grid md:px-14 mt-10 md:w-2/3 md:mb-10 md:pt-10 md:flex md:justify-end gap-5 md:pr-16">
          <button
            class="md:w-1/4 border border-danger text-danger font-normal py-2 rounded-xl hover:bg-danger hover:text-white transition-all duration-500"
            type="button" @click="editInformation = !editInformation">
            {{ $t("cancel") }}
          </button>
          <button
            class="shadow md:w-1/4 bg-origin hover:bg-sky-700 transition-all duration-500 text-white font-normal py-2 rounded-xl"
            type="submit" @click="settingSubmit">
            {{ $t("save") }}
          </button>
        </div>
      </div>
        ...
        <div class="flex justify-between items-center p-10">
          ...
          <teleport to="body">
            <BaseModal :isOpen="popUp" :btnDisable="btnDisable" @close-modal="togglePopUp()" @submit="submit"
              :classList="'md:w-[50vw] w-full'">
              <template #title>
                <h1 class="text-center text-2xl text-primary">
                ...
                <form class="px-5" action="">
                  <div class="md:flex w-full justify-around gap-3 p-5 text-primary">
                    <div class="flex flex-col md:w-2/4">
                      ...
                      <input id="companyName" class="rounded-xl capitalize" :class="$errorBorder('name')" type="text"
                        :placeholder="$t('companyName')" v-model="form.name" />
                      <em :class="$errorText('name')">{{ errors.name }}</em>
                    </div>
                    <div class="flex flex-col md:w-1/4">
                      <label class="pb-1" for="managementLevel">{{ $t('enterpriseLevel') }}</label>
                      <select id="managementLevel" class="rounded-xl capitalize"
                        :class="$errorBorder('management_level_id')" v-model="form.management_level_id" required>
                  ...
                  <div class="text-primary px-5 mb-8">
                    <h1 class="block w-full">{{ $t('holiday') }} :</h1>
                    <div class="flex flex-wrap justify-normal md:justify-evenly gap-3">
                      <div>
                      ...
                      <div>
                        <input class="w-6 h-6 text-lightGreen bg-gray-100 rounded-lg" type="checkbox"
                          v-model="form.special_leave" id="special_leave" true-value="1" false-value="0" />
                        <label class="pl-2" for="special_leave">{{ $t('specialLeave') }}</label>
                      </div>
                      <div>
                        <input class="w-6 h-6 text-lightGreen bg-gray-100 rounded-lg" type="checkbox"
                          v-model="form.summer_leave" id="summer_leave" true-value="1" false-value="0" />
                      ...
                      <div>
                        <input class="w-6 h-6 text-lightGreen bg-gray-100 rounded-lg" type="checkbox"
                          v-model="form.holiday" id="holiday" true-value="1" false-value="0" />
...
export default {
  ...
  methods: {
    ...
    async settingSubmit() {
      ...
      await this.axios
        .post("company-setting/update/" + this.formSetting.id, payload)
        .then((response) => {
          if (response.data.success) {
            this.$swal({
              icon: "success",
              html: `<h1>แก้ไขสำเร็จ!</h1> <br/> <span class='text-sm'>${response.data.message}</span>`,
              confirmButtonText: "ตกลง",
            }).then(() => {
        ...
        .catch((err) => {
          ...
          this.$emit('reloadCompanyProfile', this.formSetting.id)
        })
    },
    edit(id) {
      this.loading = true;
